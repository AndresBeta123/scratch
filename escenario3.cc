...
int capacidadCap2=1;
int numClustersCap1 = 8; // Escenario 3
int numClustersCap2 = 3;
int numNodesCap1 = 6;
//int numNodesCap2 = 4;
...
//randomArrival.SetAttribute ("Mean", DoubleValue (1.0 / interval));
  ...
  /*Este escenario tenía 8 cluster con 6 nodos en la primera capa y 3 cluster de 4 nodos
  en la segunda capa. */
  ...
  // Segunda capa
  // cluster 0 con cabezas 0, 3, 6
  // cluster 1 con cabezas 1, 4, 7
  // cluster 2 con cabezas 2, 5
  NodeContainer c2Cluster[numClustersCap2];
  for(int i = 0; i < numClustersCap2; i++) {
    c2Cluster[i].Create(1);
    if(i == 2) c2Cluster[i].Create(1);   // Crear uno extra para los clusters con solo 1 cabeza
    c2Cluster[i].Add(c1Cluster[i].Get(0));
    c2Cluster[i].Add(c1Cluster[i+numClustersCap2].Get(0));
  }

  c2Cluster[0].Add(c1Cluster[6].Get(0));
  c2Cluster[1].Add(c1Cluster[7].Get(0));
  ...
  wifiPhy[0].EnableAsciiAll(ascii.CreateFileStream("escenario3salida.tr"));
